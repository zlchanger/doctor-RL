...
def prepare_prompts(samples: List[Dict], tokenizer) -> List[str]:
    ...
    for sample in samples:
        self_report = sample['prompt'][0]['content'].split("Patient's description:")[1].split("\n\n\nDecide next action:")[0]
        dialogue = sample['reward_model']['patient_information']
        diagnosis = sample['reward_model']['ground_truth']['diagnosis']
        recommendation = sample['reward_model']['ground_truth']['recommendation']
        ...
        prompt = (
            ...
            "\nRules:\n"
            "1. Summarize the patient's information: Combine the 'Original self_report' and all patient responses from 'dialogue' into a single coherent paragraph. "
...
def process_samples_in_worker(rank, world_size, model_path, input_file, output_file, batch_size=16):
    ...
    for batch_start in tqdm(range(0, len(worker_data), batch_size), desc=f"GPU {rank} processing"):
        ...
        try:
            ...
            for i, sample in enumerate(batch_samples):
                sample['reward_model']['enhanced_description'] = enhanced_descriptions[i]
                local_output_data.append(sample)
                
        except Exception as e:
            print(f"Warning: GPU {rank} skipping batch {batch_start}-{batch_end} due to error: {e}")
            for sample in batch_samples:
                sample['reward_model']['enhanced_description'] = ""
                local_output_data.append(sample)
    
    # Save final results for this worker
    final_output_file = output_file.replace(".parquet", f"_rank{rank}_final.parquet")
    pd.DataFrame(local_output_data).to_parquet(final_output_file)
    ...
    if rank == 0:
        ...
        for r in range(1, world_size):
            other_rank_file = output_file.replace(".parquet", f"_rank{r}_final.parquet")
            other_data = pd.read_parquet(other_rank_file).to_dict('records')
